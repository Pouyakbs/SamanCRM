...
namespace SamanCRM.Core.ApplicationService.Features.PersonsFeatures.Commands
{
    public class CreatePersonCommand : IRequest<int>
    {
        ...
        public class CreatePersonCommandHandler : IRequestHandler<CreatePersonCommand, int>
        {
            ...
            public CreatePersonCommandHandler(IPersonsFacade personsFacade)
            {
...
#pragma warning disable CS1998 // Async method lacks 'await' operators and will run synchronously
            public async Task<int> Handle(CreatePersonCommand command, CancellationToken cancellationToken)
            {
                ...
                personsDTO.CreatedDate = DateTime.Now;
                personsDTO.PersonGuid = Guid.NewGuid();
                personsFacade.Add(personsDTO);
