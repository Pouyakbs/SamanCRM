...
{
    public class UpdatePreInvoiceCommand : IRequest<int>
    {
        ...
        public class UpdatePreInvoiceCommandHandler : IRequestHandler<UpdatePreInvoiceCommand, int>
        {
            private readonly IPreInvoiceFacade preInvoiceFacade;

            public UpdatePreInvoiceCommandHandler(IPreInvoiceFacade preInvoiceFacade)
            {
                this.preInvoiceFacade = preInvoiceFacade;
            }
#pragma warning disable CS1998 // Async method lacks 'await' operators and will run synchronously
            public async Task<int> Handle(UpdatePreInvoiceCommand command, CancellationToken cancellationToken)
            {
                var preInvoiceDTO = new PreInvoiceDTO();
                preInvoiceDTO.PreInvoiceID = command.PreInvoiceID;
                ...
                preInvoiceFacade.Update(preInvoiceDTO);
