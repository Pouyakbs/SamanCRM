...
function CluesInfoCard({ title, description, info, shadow , getData }) {
  ...
  function removeClue(id) {
    if(id) {
      let isSuccess = false;
      axios
        .delete(`${appConfig.BaseURL}/api/clues/${id}`)
        .then((res) => {
  ...
  const renderItems = values.map((items, key) => (
    ...
    <MDBox key={key} display="flex" py={1} pr={2}>
      <Alert variant="filled" icon={<MonitorOutlined/>} severity="info" sx={{width: "100%" , backgroundColor: "rgb(252 24 219 / 74%)", display:'flex', alignItems: "center"}}>
        &nbsp;{clues.firstName} {clues.lastName}
        <div>
        <Tooltip title={t("ویرایش")} placement="top">
            <IconButton
              ...
              onClick={() => transferToForm(clues.id)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
...
CluesInfoCard.defaultProps = {
  shadow: true,
};

// Typechecking props for the ProfileInfoCard
CluesInfoCard.propTypes = {
  title: PropTypes.string.isRequired,
  info: PropTypes.objectOf(PropTypes.string).isRequired,
  shadow: PropTypes.bool,
};

export default CluesInfoCard;
