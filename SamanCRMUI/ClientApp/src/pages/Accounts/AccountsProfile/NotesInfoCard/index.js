...
function NotesInfoCard({ title, description, info, shadow, getData }) {
  ...
  const transferToForm = (id) => {
    history.navigate(`/Activities/note/createNote?id=${id}`);
  };
  ...
  const renderItems = values.map((items, key) => (
    ...
    <MDBox key={key} display="flex" py={1} pr={2}>
      <Alert variant="filled" icon={<OrderedListOutlined/>} severity="info" sx={{width: "100%" , backgroundColor: "rgb(25 162 255/ 74%)", display:'flex', alignItems: "center"}}>
        &nbsp;{dutys.subject}
        <div>
...
NotesInfoCard.defaultProps = {
  shadow: true,
};

// Typechecking props for the ProfileInfoCard
NotesInfoCard.propTypes = {
