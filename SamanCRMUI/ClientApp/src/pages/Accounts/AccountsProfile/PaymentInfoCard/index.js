...
function PaymentInfoCard({ title, description, info, shadow , getData }) {
  ...
  function removePayment(id) {
    if(id) {
      let isSuccess = false;
      axios
        .delete(`${appConfig.BaseURL}/api/payment/${id}`)
        .then((res) => {
  ...
  const transferToForm = (id) => {
    history.navigate(`/sale/Payment/createPayment?id=${id}`);
  };
  ...
  const renderItems = values.map((items, key) => (
    items.map((payment, key) => (
    <MDBox key={key} display="flex" py={1} pr={2}>
      <Alert variant="filled" icon={<CreditCardOutlined/>} severity="info" sx={{width: "100%" , backgroundColor: "rgb(204 153 255 / 74%)", display:'flex', alignItems: "center"}}>
        &nbsp;{payment.paymentName}
        <div>
        <Tooltip title={t("ویرایش")} placement="top">
            <IconButton
              ...
              onClick={() => transferToForm(payment.id)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
        ...
        <Box sx={style} style={{ textAlign: "center", width: "450px" }}>
          ...
          <div className="d-flex justify-content-center">
            <Button
              variant="contained"
              color={"success"}
              onClick={() => removePayment(activityID)}
              startIcon={
                <DoneIcon
                  style={
                    i18n.dir() === "rtl"
