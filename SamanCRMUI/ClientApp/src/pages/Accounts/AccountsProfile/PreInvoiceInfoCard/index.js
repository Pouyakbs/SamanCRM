...
function PreInvoiceInfoCard({ title, description, info, shadow , getData }) {
  ...
  function removePreInvoice(id) {
    if(id) {
      let isSuccess = false;
      axios
        .delete(`${appConfig.BaseURL}/api/preinvoice/${id}`)
        .then((res) => {
  ...
  const transferToForm = (id) => {
    history.navigate(`/sale/PreInvoice/NewPreInvoice?id=${id}`);
  };
  ...
  const renderItems = values.map((items, key) => (
    items.map((preInvoice, key) => (
    <MDBox key={key} display="flex" py={1} pr={2}>
      <Alert variant="filled" icon={<FileDoneOutlined/>} severity="info" sx={{width: "100%" , backgroundColor: "rgb(153 204 0 / 74%)", display:'flex', alignItems: "center"}}>
        &nbsp;{preInvoice.title}
        <div>
        <Tooltip title={t("ویرایش")} placement="top">
            <IconButton
              ...
              onClick={() => transferToForm(preInvoice.id)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
        ...
        <Box sx={style} style={{ textAlign: "center", width: "450px" }}>
          ...
          <div className="d-flex justify-content-center">
            <Button
              variant="contained"
              color={"success"}
              onClick={() => removePreInvoice(activityID)}
              startIcon={
                <DoneIcon
                  style={
                    i18n.dir() === "rtl"
...
PreInvoiceInfoCard.defaultProps = {
  shadow: true,
};

// Typechecking props for the ProfileInfoCard
PreInvoiceInfoCard.propTypes = {
  title: PropTypes.string.isRequired,
  info: PropTypes.objectOf(PropTypes.string).isRequired,
  shadow: PropTypes.bool,
};

export default PreInvoiceInfoCard;
