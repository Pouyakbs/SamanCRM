...
function OpportunitiesInfoCard({ title, description, info, shadow , getData }) {
  ...
  function removeOpportunity(id) {
    if(id) {
      let isSuccess = false;
      axios
        .delete(`${appConfig.BaseURL}/api/opportunities/${id}`)
        .then((res) => {
  ...
  const transferToForm = (id) => {
    history.navigate(`/sale/Clues/NewClue?id=${id}`);
  };
  ...
  const renderItems = values.map((items, key) => (
    items.map((opportunity, key) => (
    <MDBox key={key} display="flex" py={1} pr={2}>
      <Alert variant="filled" icon={<BulbOutlined/>} severity="info" sx={{width: "100%" , backgroundColor: "rgb(246 138 30 / 74%)", display:'flex', alignItems: "center"}}>
        ...
        <Tooltip title={t("ویرایش")} placement="top">
            <IconButton
              ...
              onClick={() => transferToForm(opportunity.id)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
        ...
        <Box sx={style} style={{ textAlign: "center", width: "450px" }}>
          ...
          <div className="d-flex justify-content-center">
            <Button
              variant="contained"
              color={"success"}
              onClick={() => removeOpportunity(activityID)}
              startIcon={
                <DoneIcon
                  style={
                    i18n.dir() === "rtl"
...
OpportunitiesInfoCard.defaultProps = {
  shadow: true,
};

// Typechecking props for the ProfileInfoCard
OpportunitiesInfoCard.propTypes = {
  title: PropTypes.string.isRequired,
  info: PropTypes.objectOf(PropTypes.string).isRequired,
  shadow: PropTypes.bool,
};

export default OpportunitiesInfoCard;
