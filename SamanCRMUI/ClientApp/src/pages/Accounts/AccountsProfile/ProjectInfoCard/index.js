...
function ProjectInfoCard({ title, description, info, shadow , getData }) {
  ...
  function removeProject(id) {
    if(id) {
      let isSuccess = false;
      axios
        .delete(`${appConfig.BaseURL}/api/project/${id}`)
        .then((res) => {
  ...
  const transferToForm = (id) => {
    history.navigate(`/Cooperation/Project/CreateProject?id=${id}`);
  };
  ...
  const renderItems = values.map((items, key) => (
    ...
    <MDBox key={key} display="flex" py={1} pr={2}>
      <Alert variant="filled" icon={<ProjectOutlined/>} severity="info" sx={{width: "100%" , backgroundColor: "rgb(193 193 44 / 74%)", display:'flex', alignItems: "center"}}>
        &nbsp;{project.name}
        <div>
        <Tooltip title={t("ویرایش")} placement="top">
            <IconButton
              ...
              onClick={() => transferToForm(project.id)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
        ...
        <Box sx={style} style={{ textAlign: "center", width: "450px" }}>
          ...
          <div className="d-flex justify-content-center">
            <Button
              variant="contained"
              color={"success"}
              onClick={() => removeProject(activityID)}
              startIcon={
                <DoneIcon
                  style={
                    i18n.dir() === "rtl"
