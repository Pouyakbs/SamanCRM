...
import { IconPencil, IconSquarePlus } from "@tabler/icons";
import AnimateButton from "../../../components/@extended/AnimateButton";

function SessionsInfoCard({ title, description, action, info, shadow , getData }) {
  ...
  const transferToForm = (id) => {
    history.navigate(`/Activities/Meeting/CreateMeeting?id=${id}`);
  };
  ...
  const renderItems = values.map((items, key) => (
    items.map((sessions, key) => (
    <MDBox key={key} display="flex" py={1} pr={2}>
      <Alert variant="filled" icon={<FundProjectionScreenOutlined/>} severity="info" sx={{width: "100%" , backgroundColor: "rgb(81 88 221 / 74%)", display:'flex', alignItems: "center"}}>
        &nbsp;{sessions.subject}
        <div>
        <Tooltip title={t("ویرایش")} placement="top">
            <IconButton
              ...
              onClick={() => transferToForm(sessions.id)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
        ...
        <Box sx={style} style={{ textAlign: "center", width: "450px" }}>
          ...
          <div className="d-flex justify-content-center">
            <Button
              variant="contained"
              color={"success"}
              onClick={() => removeSession(activityID)}
              startIcon={
                <DoneIcon
                  style={
                    i18n.dir() === "rtl"
...
SessionsInfoCard.defaultProps = {
  shadow: true,
};

// Typechecking props for the ProfileInfoCard
SessionsInfoCard.propTypes = {
