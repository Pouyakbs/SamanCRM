...
const ActionCell=(props)=>{
    console.log("props injast" , props)
    ...
    const removePersonnel = () => {
        let isSuccess = false;
        axios
          .delete(`${appConfig.BaseURL}/api/ProductCategory/${props.dataItem.categoryID}`)
          .then((res) => {
        ...
        const transferToForm = () => {
            history.navigate(`/inventory-management/productCategories/create-category?id=${props.dataItem.categoryID}`)
        }
    const style = {
        position: 'absolute',
        top: '50%',
