...
import { Box, FormControlLabel, Modal, Tooltip, useTheme } from "@mui/material";
...
const ActionCell = (props) => {
    ...
    const removePersonnel = () => {
        let isSuccess = false;
        axios
            .delete(`${appConfig.BaseURL}/api/payment/${props.dataItem.paymentID}`)
            .then((res) => {
    ...
    const transferToForm = () => {
        history.navigate(`/sale/Payment/createPayment?id=${props.dataItem.paymentID}`);
    };
    const style = {
        position: "absolute",
        top: "50%",
