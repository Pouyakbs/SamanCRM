...
const ActionCell=(props)=>{
    ...
    const removeProduct = () => {
        let isSuccess = false;
        axios
          .delete(`${appConfig.BaseURL}/api/Products/${props.dataItem.productID}`)
          .then((res) => {
        ...
        const transferToForm = () => {
            history.navigate(`/inventory-management/product/create-product?id=${props.dataItem.productID}`)
        }
    const style = {
        position: 'absolute',
        top: '50%',
