...
function UploadFile({
  ...
  useEffect(() => {
    setFileList(defaultFiles?.length ? defaultFiles : []);
  }, [defaultFiles]);
  ...
  function addFiles(file) {
    // if (file.size < 25000000) {
      if (multiple) {
        ...
        fileTemp.forEach((element) => {
          var image = convertToBase64(element);

          image.then((val) =>
            setFileList((fileList) => [
              ...fileList,
              { file: val, fileName: element.name },
            ])
          );
        });
        ...
        var image = convertToBase64(...file);
        image.then((val) =>
          setFileList([{ file: val, fileName: file[0].name }])
        );
      }
    // } else {
    //   return swal({
    //     title: t("تنها فایل های با حجم زیر 25 مگابایت مجاز است"),
    //     icon: "error",
    //     button: t("باشه"),
    //   });
    // }
  ...
  return (
    <div className="d-flex">
      <div className="form-design p-0">
        ...
        {uploadError ? (
          <div className="error-msg">{t("بارگذاری فایل الزامی است")}</div>
        ) : null}
      </div>
      <div className="upload-files">
        {fileList?.map((item, index) => {
          console.log("item------------", item);
          let sp = item.file.split(";");
          let type = sp[0].split(":");
            ...
            >
              <div className="item">
                ...
                {type[1]?.includes("image") ? (
                  ...
                  <div className="info">{item?.fileName}</div>
                )}
              </div>
            </div>
          );
